            ...
            <div class="card">
                    ...
                    <div class="card-body">
                        <div class="d-flex">
                            <div class="ms-auto text-muted">
                                <RouterLink :to="{name: 'project_created'}" class="btn btn-primary d-sm-inline-block">
                                    Créer un projet
                                </RouterLink>
                            </div>
                        </div>
                ...
                <div class="table-responsive">
                    <table class="table card-table table-vcenter text-nowrap datatable">
                        ...
                        <tr v-for="item in projectItem" :data-category-id="item.id">
                            <td>
                                {{ item.id }}
                            </td>
                            <td>
                            ...
                            <td class="text-nowrap text-muted">
                                ...
                                <RouterLink :to="{name: 'project_edit', params: { id: item.id}}"
...
export default {
    ...
    data() {
        return {
            ...
            headerProps: [{
                breadcrumb: true,
                textBreadcrumb: 'Liste des projets',
                nameRoute: 'users',
            }],
    ...
    methods: {
        ...
        getProject(pageNum){
            ProjectsService.get(pageNum).then((resp) => {
                this.projectItem = resp.data.items;
        ...
        status(event) {
            ...
            ProjectsService.setStatus(formData({
                id: itemId,
                status: statusId,
            })).then((resp) => {
                let item = this.projectItem.findIndex(x => x.id === itemId);

                this.projectItem[item].active = statusId;
            }).catch((error) => {
            });
        },
        ...
        removeArticle(event){
            ...
            ProjectsService.remove(formData({
                id: itemId,
            })).then((resp) => {
                let item = this.projectItem.findIndex(x => x.id === itemId);

                this.projectItem.splice(item, 1)
            }).catch((error) => {
